...
import { getPositionsInfos, closeDerivioAPositions, getProductKeeperFee } from "../src/position";
...
describe("DerivioA test", function () {
  ...
  async function openDerivioA(slot0: any, tickLowerOffset: number, tickUpperOffset: number, shortLeverage: number) {
    ...
    let keeperFee = '0';
    if (shortLeverage != 0) {
      keeperFee = await getProductKeeperFee(orderManager, gmxPositionRouter, 0)
    }
  ...
  describe("PositionRouter control flow", function () {
    ...
    it("#1 open DerivioAS by Position Router", async function () {
      ...
      console.log('product keeper fee:', await getProductKeeperFee(orderManager, gmxPositionRouter, 0));
      console.log('product keeper fee:', await getProductKeeperFee(orderManager, gmxPositionRouter, 1));
      ...
      const slot0 = await uniswapV3Pool.slot0();
      await openDerivioA(slot0, -25, 10, 0);
    ...
    it("#2 open DerivioAL by Position Router", async function () {
      ...
      const slot0 = await uniswapV3Pool.slot0();
      await openDerivioA(slot0, -25, 10, 1000000);
      console.log('positionsInfos:', JSON.stringify(await getPositionsInfos(derivioPositionManager, owner.address), null, 2));
    });
    ...
    it("#3 close DerivioAS by Position Router", async function () {
      ...
      const slot0 = await uniswapV3Pool.slot0();
      await openDerivioA(slot0, -250, 100, 0);
      ...
      await closeDerivioAPositions(positionRouter, [positionKeys[0]], [0], true, weth.address, usdc.address);
      // await positionRouter.closeDerivioPosition([positionKeys[0]], 'true', weth.address, usdc.address);
    ...
    it("#4 close DerivioAL by Position Router", async function () {
      ...
      const slot0 = await uniswapV3Pool.slot0();
      await openDerivioA(slot0, -25, 10, 5000000);
      ...
      await closeDerivioAPositions(positionRouter, [positionKeys[0]], [0.0001], true, weth.address, usdc.address);
    ...
    // it("#11 Should be open DerivioAL with leverage", async function () {
    ...
    it("#14 Simulate GMX liquidation", async function () {
      ...
      await gmxPositionManager.connect(liquidator).liquidatePosition(minVaultAddress, usdc.address, weth.address, isLong, liquidator.address);
      positions = await getPositionsInfos(derivioPositionManager, owner.address);
      console.log('positionsInfos:', JSON.stringify(positions, null, 2));

      await derivioA.liquidatePosition(
        owner.address, 
        {
          positionKey: positions[0].positionKey,
          swapToCollateral: true,
        },
        { value: ethers.utils.parseUnits("0.0001", 18) }
      );
